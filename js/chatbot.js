(function () {
    ...
    async function getQuestion(questionId) {
        ...
        try {
            ...
            displayBotMessage(showMessage('serverError'));
        }
    }
    ...
    sendButton.addEventListener("click", function () {
        ...
        const lastId = currentQuestion[currentQuestion.length - 1]?.id;


        if (input !== "" && !isNaN(input) && Number.isFinite(Number(input))) {
    ...
    // Mostrar las opciones disponibles (solo números y texto de las opciones)
    ...
    function displayUserMessage(message) {
        ...
        userMessage.appendChild(timeNode);  // Primero la hora
        userMessage.appendChild(messageContainer);  // Luego el mensaje con el icono
    ...
    async function getDocument(optionId) {
        ...
        try {
            ...
            console.log('Mensaje de error en el servidor', error.message);
        }
    }
